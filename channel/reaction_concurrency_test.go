...
func triggerReaction(d *dispatcher.EventDispatcher, eventID, messageID, emojiType string) {
	payload := fmt.Sprintf(`{
		"schema": "2.0",
		"header": {
			"event_id": "%s",
...
func TestReactionConcurrencyAndDedup(t *testing.T) {
	...
	t.Run("Test Dedup (去重拦截)", func(t *testing.T) {
		...
		for i := 0; i < 10; i++ {
			...
			go func() {
				defer wg.Done()
				triggerReaction(d, "identical_event_id_123", "msg_1", "DEDUP_TEST")
			}()
		}
		wg.Wait()
	...
	t.Run("Test Serialization (串行排队)", func(t *testing.T) {
		...
		// 瞬间并发发送 50 个具有 **不同 EventID** 但 **相同 MessageID** 的请求
		for i := 0; i < concurrencyLevel; i++ {
			wg.Add(1)
			go func(idx int) {
				defer wg.Done()
				eventID := fmt.Sprintf("unique_event_id_%d", idx)
				triggerReaction(d, eventID, "same_message_id_456", "SMILE")
			}(i)
		}
		wg.Wait()
